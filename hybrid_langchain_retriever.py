...
"""
hybrid_langchain_retriever.py - semantic Chroma + Neo4j-friendly hits
(Updated: defers final year ordering to database_manager)
"""
...
def get_chroma_client():
    ...
    cache_key = f"{cfg.chroma_dir}_{cfg.chroma_collection}"
    if cache_key not in _chroma_clients:
        try:
            ...
            print(f"✅ ChromaDB initialized: {cfg.chroma_collection}")
        except Exception as e:
            print(f"❌ ChromaDB initialization failed: {e}")
            _chroma_clients[cache_key] = None
    return _chroma_clients[cache_key]
...
def hybrid_search(query: str, k_chroma: int = 8, k_graph: int = 8):
    ...
    2. repackage to look graph-friendly
    """
    cfg = get_active_db_config()
    chroma = get_chroma_client()
    ...
    try:
        ...
        # we still do a local sort by year desc here to reduce work for RAG
        chroma_docs = sorted(
            chroma_docs,
            key=lambda d: _safe_int(d.metadata.get("year", 0)),
            reverse=True,
    ...
    for doc in chroma_docs:
        ...
        graph_hit["paper_id"] = (
            m.get("doi", "") or m.get("paper_id", "") or str(hash(doc.page_content))
        )
        graph_hit["title"] = m.get("title", "Untitled")
        graph_hit["year"] = _safe_int(m.get("year", 0))
        graph_hit["doi"] = m.get("doi", "")
