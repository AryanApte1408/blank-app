...
def render_graph_from_hits(graph_hits: List[Dict], height: int = 700, max_papers: int = 15):
    ...
    try:
        ...
        with driver.session(database=cfg.neo4j_database) as session:
                ...
                for path in paths:
                    ...
                    for node in path.nodes:
                        ...
                        if nid not in nodes_dict:
                            ...
                            nodes_dict[nid] = _make_node(str(nid), str(name), nlabel)
            ...
            print(f"✅ Processed {count} papers from Neo4j")
    ...
    if not nodes_dict:
        return f"Papers from ChromaDB not found in Neo4j '{cfg.neo4j_database}'.", None, None
    ...
    return (
        agraph(nodes=nodes, edges=edges, config=cfg_vis),
        cypher,
        {"identifiers": all_identifiers[:10]},
    )
